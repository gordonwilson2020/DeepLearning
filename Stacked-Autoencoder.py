...
# 4. Stacked Auto-encoder Features Extraction

# 4.1. SAE Algorithm Features
...
# 4.1.2. Target/Predictor Features
xt = prspyt[:, 1:6]

# 4.2. SAE Features Extraction Model
def saet_model(drate):
    # 4.2.1. Model Type
    saet = nn.Sequential()
    # 4.2.2. Model Layers
    ...
    # Encoder 1
    saet.add(ml.Dense(units=5, input_shape=(5,)))
    saet.add(ml.Activation(activation='linear'))
    saet.add(ml.Dropout(rate=drate))
    saet.add(ml.ActivityRegularization(l1=0.0, l2=0.0))
    # Decoder 1
    saet.add(ml.Dense(units=5))
    saet.add(ml.Activation(activation='linear'))
    saet.add(ml.Dropout(rate=drate))
    saet.add(ml.ActivityRegularization(l1=0.0, l2=0.0))
    # Encoder 2
    ...
    saeoptt = opt.SGD(lr=0.01, momentum=0, decay=0, nesterov=False)
    # 4.2.4. Model Compilation
    # Loss function options: 'mean_squared_error'
    saet.compile(loss='mean_squared_error', optimizer=saeoptt, metrics=['accuracy'])
    return saet

# 4.3. SAE Features Extraction Training Optimal Parameter Selection

# 4.3.1. SAE Time Series Cross-Validation
...
saets = time.time()
cvsaet = cv.GridSearchCV(ks.KerasRegressor(saet_model), cv=cv.TimeSeriesSplit(n_splits=5),
                          param_grid={"drate": [0.00, 0.25]}).fit(xt, xt)
saete = time.time()

# 4.3.2. SAE Time Series Cross-Validation Optimal Parameter Selection
cvsaepar = cvsaet.best_params_

# 5. Deep Neural Network Regression

# 5.1. DNN Regression Features

# 5.1.1. Features Pre-processing
# re-scaling range [0,1]
# prspyt = (rspyt-min(rspyall))/(max(rspyall)-min(rspyall))
prspyt = scalet.transform(rspyt)
...
# 5.1.3. Predictor Features Selection
xta = prspyt[:, [1, 2, 5]]
xfa = prspyf[:, [1, 2, 5]]

# 5.1.4. Predictor Features Extraction
# Stacked Auto-encoder
xf = prspyf[:, 1:6]
saett = cvsaet.best_estimator_.model
xtb = saett.predict(x=xt)
xfb = saett.predict(x=xf)

# 5.2. DNN Regression Model
def dnnt_model(drate, inshape):
    ...
    # Back-propagation optimization options: 'SGD' stochastic gradient descent
    # Optimization regularization: learning rate 'lr', 'momentum' and learning rate 'decay'
    dnnoptt = opt.SGD(lr=0.01, momentum=0, decay=0, nesterov=False)
    # 4.2.4. Model Compilation
    # Loss function options: 'mean_squared_error'
    dnnt.compile(loss='mean_squared_error', optimizer=dnnoptt, metrics=['accuracy'])
    return dnnt

# 5.3. DNN Regression Training Optimal Parameter Selection

# 5.3.1. DNN Time Series Cross-Validation
# Exhaustive Grid Search Time Series Cross-Validation with Parameter Array Specification
# TimeSeriesSplit = anchored time series cross-validation with
...
# 5.3.2. DNN Time Series Cross-Validation Optimal Parameter Selection
cvdnnpara = cvdnnta.best_params_
cvdnnparb = cvdnntb.best_params_
print("")
print("== SAE Features Extraction Training Optimal Parameter Selection ==")
print("")
print("SAE Features Extraction Optimal Dropout Rate: ", cvsaepar)
print("")
print("SAE Features Extraction Training Time: ", (saete-saets)/60, " minutes")
print("")
...
# 5.6. DNN Regression Testing Forecasting Accuracy
